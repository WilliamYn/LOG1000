...
int main(int argc, char **argv)
{
    ...
    while(choice!=0)
    {
        ...
        std::cout << "\nVeuillez choisir parmis une des options suivantes: \n"
                  << "\t0. Quitter.\n"
                  << "\t1. Consulter les disponibilités d'une infirmiere.\n"
                  << "\t2. Prendre rendez-vous avec une infirmiere\n"
        ...
        case 2:
        {
            ...
            if (appointments.schedule(loggedInUser->profileID, nurse[NURSES_USER_ID]))
            {
                ...
                std::cout << "\nVotre serez facturé " << appointments.getHoursRate(loggedInUser->disease, nurse[NURSES_EXPERIENCE_YEARS]) << " dollars";
            }
            else
            {
                std::cout << "\nCette infirmiere est indisponible pour cette période.";
            }
            // Attend que l'utilisateur appuie sur ENTER pour continuer l'exécution.
            std::cout << "\nAppuyez sur ENTER pour continuer...";
            std::cin.ignore();
